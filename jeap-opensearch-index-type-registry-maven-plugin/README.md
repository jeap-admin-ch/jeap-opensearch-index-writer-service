...
| `deployAllIndexTypes`      | `false`                                                            | When `false`, only index types with new or added mapping files compared to the trunk branch are deployed on **feature branches**. On the trunk branch all index types are always deployed regardless of this flag (git comparison is skipped because after a merge trunk already contains the changes, so a diff would find nothing and newly added types would never be deployed). Requires `gitUrl`; ignored (all deployed) if `gitUrl` is blank. |
